...
DataName = os.getcwd() + '/OrderedSample.xlsx'
SourceData = pd.read_excel(DataName, engine='openpyxl')
# Loop through all rows in the "預測資料檔名" column
for index, file_name in enumerate(SourceData["預測資料檔名"]):
    print(f"Row {index + 1}: {file_name}")

    # 設定LSTM往前看的筆數和預測筆數
    LookBackNum = 12 # LSTM往前看的筆數
    ForecastNum = 48 # 預測題目當天的筆數 9:00至16:59

    # Replace the last 'N' with '01' if it ends with 'N'
    if file_name.endswith('N.csv'):
        file_name = file_name.replace('N.csv', '01.csv')
        print(f"Row {index + 1}: Updated file_name = {file_name}")

    # 載入訓練資料
    # DataName = os.getcwd() + '/TrainingData/L17_0331_11.csv'
    DataName = os.getcwd() + '/segmentedSamples' + '/' + file_name + '.csv'
