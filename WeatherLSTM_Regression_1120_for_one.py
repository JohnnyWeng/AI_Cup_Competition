...
# data_file = os.getcwd() + '/比賽題目.csv'
# competition_data = pd.read_csv(data_file, encoding='utf-8')
# # competition_data = competition_data[['題號', '預測資料檔名']]
...
FileName = "D030_L04_0420_02.csv"
DataName = os.getcwd() + '/TrainingData/'+FileName
try:
  SourceData = pd.read_csv(DataName, encoding='utf-8')
  print(DataName)
  # print(SourceData.shape)
except Exception as e:
  print(f"讀取檔案失敗：{DataName}")
  print(f"錯誤訊息：{e}")
...
regressor.add(LSTM(units=256, return_sequences=True, input_shape=(X_train.shape[1], 5)))
...
#寫預測結果寫成新的CSV檔案
# 將陣列轉換為 DataFrame
df = pd.DataFrame(PredictPower)

# 將 DataFrame 寫入 CSV 檔案
df.to_csv(f"{FileName[:16]}.csv", index=False, header=False, encoding='utf-8-sig')
